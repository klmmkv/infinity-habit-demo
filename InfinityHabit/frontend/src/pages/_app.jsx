...
import InfinityHabbitJson from "data/abis/InfinityHabit.json";
...
function MyApp({ Component, pageProps }) {
  ...
  const load = async () => {
    ...
    const habitContract = getSignedContract(
      "0x09635f643e140090a9a8dcd712ed6285858cebef",
      InfinityHabbitJson.abi
    );

    if (!habitContract) return;

    const currentAccount = await getCurrentAccount();
    setContracts({habitContract });
